...
impl NetworkController {
    pub fn new(service: String, listen_addr: SocketAddr, timeout_ms: u64) -> Self {
        ...
        Self {
            inbound_handler,
            outbound_handler,
            inbound_rpc_runtime: runtime::Builder::new_multi_thread().worker_threads(1).enable_all().thread_name("inbound_rpc").build().unwrap(),
            outbound_rpc_runtime: Arc::new(runtime::Builder::new_multi_thread().worker_threads(1).enable_all().thread_name("outbound_rpc").build().unwrap()),
