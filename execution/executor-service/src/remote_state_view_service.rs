...
impl<S: StateView + Sync + Send + 'static> RemoteStateViewService<S> {
    pub fn new(
        ...
        let num_threads = 144; // 3 * remote_shard_addresses.len();//remote_shard_addresses.len() * 2; //num_threads.unwrap_or_else(num_cpus::get);
    ...
    pub fn start(&self) {
        ...
        let num_handlers = 144; //6 * self.num_shards;
