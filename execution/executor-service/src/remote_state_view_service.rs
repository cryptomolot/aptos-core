...
impl<S: StateView + Sync + Send + 'static> RemoteStateViewService<S> {
    pub fn new(
        ...
        let num_threads = 2 * remote_shard_addresses.len();//remote_shard_addresses.len() * 2; //num_threads.unwrap_or_else(num_cpus::get);
    ...
    pub fn start(&self) {
        ...
        let thread_pool_clone = 2 * self.thread_pool.clone();
        let num_handlers = self.num_shards;
