...
impl<S: StateView + Sync + Send + 'static> RemoteExecutorClient<S> {
    pub fn new(
        ...
        .for_each(|(shard_id, address)| {
            ...
            for _ in 0..std::cmp::max(1,num_threads/(10 * num_shards)) {
...
impl<S: StateView + Sync + Send + 'static> ExecutorClient<S> for RemoteExecutorClient<S> {
    ...
    fn execute_block_remote(
        ...
        //     let mut to_receive = 0;
        //     let (send_finished, recv_finished) = crossbeam_channel::unbounded();
        //     let send_finished = Arc::new(send_finished);
        ...
        //         let send_finished_clone = send_finished.clone();
        ...
        //             // });
        //             send_finished_clone.send(()).unwrap();
        //         });
        //     }
        //     while let Ok(msg) = recv_finished.recv() {
        //         to_receive -= 1;
        //         if to_receive == 0 {
        //             break;
        //         }
